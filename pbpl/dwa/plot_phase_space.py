...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog='Example:\n' +
        '  > pbpl-dwa-plot-phase-space plot-phase-space.toml result.h5\n' +
...
def plot_frame(output, data, step, conf_xaxis, conf_yaxis, aeval):
    xvalue = conf_xaxis['value']
    try:
        foo = aeval(xvalue)
    except NameError:
        print('A')
...
#     ax.plot(
# #        aeval(conf_xaxis_value), aeval(args.yaxis_value),
#         marker='o', ls='', markersize=3, markeredgewidth=0,
#         color='#0083b8', alpha=0.5)

    ax.set_xlabel(conf_xaxis['title'], labelpad=0.0)
    ax.set_ylabel(conf_yaxis['title'], labelpad=0.0)

    ax.set_xlim(conf_xaxis['min'], conf_xaxis['max'])
    ax.set_ylim(conf_yaxis['min'], conf_yaxis['max'])
...
def main():
    ...
    for i in range(num_steps):
        plot_frame(
            output, data, i, conf['xaxis'], conf['yaxis'], aeval)
    output.close()
